 ...
 * 		- userRepository 프로퍼티는 UserRepository 인터페이스 타입의 객체를 의존성주입 받는다.
 * 		- UserRepository는 인터페이스 타입이다.
 * 		- userRepository 프로퍼티에는 UserRepository 인터페이스의 구현객체가 의존성주입된다.
...
public class UserService {
	...
	/**
	 * 신규 사용자 정보가 저장된 UserSignupForm 객체를 전달받아서 사용자 정보를 회원가입 처리한다.
	...
	public User getUser(Long id) {
		...
		/*
		 * if (optional.isEmpty()) { throw new RuntimeException("사용자 정보가 존재하지 않습니다."); }
		 * return optional.get();
		 */
	}
