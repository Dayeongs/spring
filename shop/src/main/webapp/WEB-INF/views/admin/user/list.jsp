...
<script type="text/javascript">
	...
	function showUserInfo(id) {
		...
		xhr.onreadystatechange = function() {					// 2. xhr 객체의 readyState 값이 변경될 때마다 실행되는 이벤트핸들러 함수를 등록한다.
			if (xhr.readyState == 4 && xhr.status == 200) {		// 5. readyState=4: 응답이 왔다, status=200: 성공
				...
				console.log(text);
				let user = JSON.parse(text);					// 7. user = {no:1001, id:"hong", name="홍길동", ...}, 역직렬화
		...
		xhr.open("GET", "/admin/users/" + id);					// 3. xhr 객체를 초기화한다. 요청방식, 요청 URL을 지정한다.
